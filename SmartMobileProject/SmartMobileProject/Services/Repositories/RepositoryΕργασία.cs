...
{
    class RepositoryΕργασία : IDataControl
    {
        ...
        public async Task<bool> UpdateItemAsync()
        {
            try
            {
                ...
                using (var uow = XpoHelper.CreateUnitOfWork())
                {
                    ...
                    {
                        return uow.Query<Εργασία>().Where(
                        x => x.Πωλητής.SmartOid == App.Πωλητής.SmartOid
                        && x.Κατάσταση
                        && x.IsUploaded == false).ToList<Εργασία>();
                    });
                    if (allItemsList.Any())
                    {
                        var jsonlist = await ConvertToJson(allItemsList);
                        var can = await XpoHelper.setSmartTable(jsonlist,"Tasks");
        ...
        Task<string> ConvertToJson(List<Εργασία> items)
        {
            var task = Task<string>.Run(() =>
            {
                string json = "{\"Εργασίες\":[";
